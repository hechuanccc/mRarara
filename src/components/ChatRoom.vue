  ...
  <div class="chat-box" id="chatBox" :style="{backgroundImage: `url(${systemConfig.mobileBackground})`}">
    ...
    <div v-else class="chat-body">
      <div class="chat-content" id="chatContent" @click="showSmile = false">
        <ul class="lay-scroll">
          <li v-for="(item, index) in $store.state.messages" :class="['clearfix', 'item', item.sender && user.username === item.sender.username ? 'item-right' : 'item-left', item.type < 0 ? 'sys-msg' : '']">
